...
def capability_required(*cap_keys):
    """已登录；pending 去等待页；admin 全放行；否则需具备任一能力键（OR）。"""
    if not cap_keys:
        raise ValueError("capability_required 至少需要一个能力键")
    ...
    def decorator(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            if code == "admin":
                return f(*args, **kwargs)
            if any(current_user_can_cap(k) for k in cap_keys):
                return f(*args, **kwargs)
